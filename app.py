...
if option == 'Company Info':
    company= get_symbol(ticker)
    st.subheader(company)
    st.text("Address:"+str(yf.Ticker(ticker).info['address1']+ ", "+ yf.Ticker(ticker).info['city']+ ", "+ yf.Ticker(ticker).info['state']+ ", "+ yf.Ticker(ticker).info['country']+ " - "+ yf.Ticker(ticker).info['zip']))
